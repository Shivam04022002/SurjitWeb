...
const articleImage = require('../images/articleImage.service');
const branding = require('../images/branding.service');
...
const generateBlog = ({ topic, createDate, category, tags, seoKeywords, rowKey }, userId) =>
    exclusive(jobKey(userId, 'blog', rowKey), userId, async () => {
        ...
        return {
            ...
            createDate,
            model,
            // The Gemini key is set (generation needs it), so an AI image can
            // be made whenever the API-page toggle is on.
...
// Featured images: generated by the Gemini image model for this article and
// branded with the real Surjit Finance logo. If that fails and the API page
// allows it, a Pexels photo (also branded) is used instead and labelled as
// such — never passed off as AI-generated. There is no image *model*
// fallback, and the text models' rest/fallback state is not involved.
// A failure here is non-fatal for the blog: the admin can upload an image.
//
// Time: the whole request stays within GEMINI_TIMEOUT_MS. When Pexels may be
// needed, the AI image gets less of it so the fallback still fits.
const PEXELS_RESERVE_MS = 15000;
const BRANDING_RESERVE_MS = 3000;
const MIN_AI_MS = 1000;

const encoded = (branded) => ({
    mimeType: branded.mimeType,
    data: branded.buffer.toString('base64'),
    size: branded.buffer.length,
    width: branded.width,
...
const generateImage = ({
    title, summary, topic, content, category, tags, imagePrompt, variation = 0, excludePhotoIds = [], rowKey
}, userId) =>
    exclusive(jobKey(userId, 'image', rowKey), userId, async () => {
        ...
        const deadline = Date.now() + env.GEMINI_TIMEOUT_MS;
        const pexelsKey = config.pexelsFallbackEnabled ? await pexelsConfig.resolveApiKey() : null;

        let aiError;
        try {
            const reserve = pexelsKey ? PEXELS_RESERVE_MS : BRANDING_RESERVE_MS;
            const image = await articleImage.generateArticleImage(
                { title, topic, excerpt: summary, content, category, keywords: tags, scene: imagePrompt },
                { apiKey: config.apiKey, model: config.imageModel, variation, timeoutMs: Math.max(MIN_AI_MS, deadline - Date.now() - reserve) }
            );
            return {
                ...encoded(image),
                provider: 'gemini',
                model: config.imageModel,
                modelName: imageModels.displayName(config.imageModel),
                generatedFor: 'article',
                variation
            };
        } catch (err) {
            aiError = err;
        ...
        if (!pexelsKey) throw aiError;
        if (deadline - Date.now() < 5000) {
            throw new AppError(`${aiError.message} There was no time left to try the Pexels fallback.`, aiError.statusCode || HTTP_STATUS.BAD_GATEWAY);
        }
        logger.warn('AI featured image failed; using the Pexels fallback', { status: aiError.statusCode || null, model: config.imageModel });
        try {
            const photo = await pexels.findImage({ title, topic: topic || summary, imagePrompt, excludeIds: excludePhotoIds, apiKey: pexelsKey });
            // Stock photos come in many shapes (often 3:2): centre-cropped to
            // exactly 16:9 like the AI images, then branded.
            const branded = branding.brandImage(Buffer.from(photo.data, 'base64'), { crop: '16:9' });
            return {
                ...encoded(branded),
                provider: 'pexels',
                source: 'pexels',
                query: photo.query,
                credit: photo.credit,
                // Shown in Review, so the admin knows why this is not AI-made.
                notice: `AI image unavailable (${aiError.message}) — a Pexels photo was used instead.`
            };
        } catch (err) {
            throw new AppError(`${aiError.message} The Pexels fallback also failed: ${err.message}`, aiError.statusCode || HTTP_STATUS.BAD_GATEWAY);
        }
    });
...
const createDraft = async (data, files) => {
    const { createDate, imageCredit, imageMeta, ...fields } = data;

    // Where the featured image came from is stored with it: an AI image
    // (provider, model, branded), or a Pexels photo with its photographer
    // credit. Only with an uploaded featured image; a manual upload carries
    // none of it.
    if (files.featuredImage && (imageMeta || imageCredit)) {
        const meta = imageMeta || {};
        const credit = meta.provider === 'gemini' ? null : imageCredit;
        files = {
            ...files,
            featuredImage: {
                ...files.featuredImage,
                ...(meta.provider ? { provider: meta.provider } : credit ? { provider: 'pexels' } : {}),
                ...(meta.provider === 'gemini' ? { model: meta.model, generatedFor: 'article' } : {}),
                ...(meta.branded !== undefined ? { branded: meta.branded } : {}),
                ...(credit ? { credit } : {})
            }
        };
    }
